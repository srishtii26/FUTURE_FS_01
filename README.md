# ✨ Srishti S Rao – Professional Portfolio Website

A modern and responsive personal portfolio website designed to showcase my skills, projects, certifications, technical experience, and professional journey in AI/ML, Embedded Systems, IoT, Cybersecurity, and Full Stack Development.

🌐 Live Website:
https://srishtii26.github.io/FUTURE_FS_01/

---

# 🚀 Overview

This portfolio website was built as a professional digital presence to represent my technical skills, engineering background, and development projects.

The website highlights:

* Technical skills and technologies
* Real-world projects
* Certifications and achievements
* Professional experience
* Career interests and goals
* Contact and networking information

The goal of this portfolio is to create a strong professional identity for recruiters, internship opportunities, freelance work, and networking.

---

# 💡 Purpose

A portfolio website helps recruiters and clients understand:

* What I can build
* My technical interests and expertise
* My project experience
* My professional presentation skills
* My learning journey and goals

This project was created as a real-world professional portfolio rather than just an academic assignment.

---

# 🎯 Features

* 👩‍💻 Professional Developer Portfolio
* 📱 Fully Responsive Design
* ✨ Modern Glassmorphism UI
* 📂 Projects Showcase Section
* 🛠️ Skills & Tech Stack Display
* 📜 Certifications & Achievements
* 📄 Resume Download Feature
* 📬 Contact Section
* ⚡ Smooth Animations & Effects
* 🔍 SEO-Friendly Meta Tags

---

# 🛠️ Technologies Used

* HTML5
* CSS3
* JavaScript

---

# 🎨 UI/UX Highlights

* Modern glassmorphism interface
* Responsive layouts for all devices
* Smooth scrolling and animations
* Professional color palette
* Clean typography and spacing
* Interactive typing animation
* User-friendly navigation structure

---

# 📂 Website Sections

* Home
* About Me
* Education
* Tech Stack
* Currently Learning
* Projects
* Skills
* Certifications
* Strengths
* Achievements
* Interests
* Contact

---

# 📸 Screenshots

<img width="960" height="504" alt="image" src="https://github.com/user-attachments/assets/6b0ff373-1f47-4c6a-a291-3a1ed5e64416" />
<img width="960" height="504" alt="image" src="https://github.com/user-attachments/assets/c548ccf1-2a65-4bab-92aa-e38f725dccbb" />
<img width="960" height="504" alt="image" src="https://github.com/user-attachments/assets/c0f08171-8216-4308-8437-3646baa3a737" />
<img width="960" height="504" alt="image" src="https://github.com/user-attachments/assets/58c3c986-963d-4eb7-bcbd-bc9cefa6843e" />
<img width="960" height="504" alt="image" src="https://github.com/user-attachments/assets/45620448-fb08-4942-99c5-198c7262cdfc" />


# 🌐 Deployment

The portfolio can be deployed using platforms such as:

* Vercel
* Netlify
* GitHub Pages

Deployment Steps:

1. Push the project to GitHub
2. Connect repository to hosting platform
3. Deploy the project
4. Share the live website link

---

# 📌 Future Improvements

* 🌙 Dark/Light Mode Toggle
* 📬 Functional Backend Contact Form
* 📊 Project Analytics Dashboard
* 📝 Blog Section
* 🔗 Live Project Demonstrations
* 🤖 AI Chat Assistant Integration

---

# 🧠 Learning Outcomes

* Improved frontend development skills
* Better understanding of responsive design
* Learned professional portfolio structuring
* Improved UI/UX design practices
* Enhanced JavaScript interaction handling
* Explored SEO optimization basics

---

# 💼 Professional Perspective

This portfolio represents my journey as an aspiring engineer and developer working in:

* Artificial Intelligence & Machine Learning
* Embedded Systems & IoT
* Cybersecurity
* Software Development
* Full Stack Web Development

The project demonstrates both technical capability and professional presentation.

---

# 📬 Contact

📧 Email: [srishtisrao26@gmail.com](mailto:srishtisrao26@gmail.com)

💼 LinkedIn:
https://www.linkedin.com/in/srishti-s-rao-b09189316

💻 GitHub:
https://github.com/srishtii26

---

# ⭐ Support

If you like this project, consider giving it a ⭐ on GitHub!

---

# 👩‍💻 Developed By

**Srishti S Rao**

*B.Tech – Electronics and Computer Science Engineering (AI/ML Specialization)*

*Christ University (Deemed to be University)*

*Bengaluru, India*

